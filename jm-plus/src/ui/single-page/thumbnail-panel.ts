...
export function createThumbnailPanel(
  ...
  const activeItems = new Map<number, HTMLDivElement>();
  ...
  function acquireItem(): HTMLDivElement {
    return itemPool.pop() || (() => {
  ...
  function releaseItem(el: HTMLDivElement): void {
    el.remove();
    itemPool.push(el);
  }

  function renderItemContent(el: HTMLDivElement, index: number): void {
