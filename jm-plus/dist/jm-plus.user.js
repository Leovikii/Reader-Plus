...
// @version            1.0.0
// @author             Viki
// @description        Reader mode with image navigation and floating controls for 18comic
// @description:zh-CN  为禁漫天堂提供阅读器模式，支持图片导航与悬浮控制
// @license            MIT
// @match              *://18comic.ink/photo/*
...
__publicField(this, "currentImageIndex", 0);
  ...
  async function descrambleImage(img, sliceCount) {
  ...
  const cache = new Map();
  async function resolveImageUrl(page, aid) {
  ...
  (async function main() {
    ...
    const scrambled = pageImages.map((p, i) => ({ ...p, index: i })).filter((p) => p.needsDescramble);
